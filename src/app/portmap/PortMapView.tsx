...
// 구분자 그룹 키: 위치(location_name) 우선, 없으면 자산유형(asset_type)으로 묶는다. 접이식 그룹용.
const deviceTypeLabels: Record<string, string> = {
  network: "네트워크", server: "서버", security: "보안",
};
function deviceGroupKey(a: { location_name?: string | null; asset_type?: string | null }): string {
  const loc = (a.location_name || "").trim();
  if (loc) return loc;
  const t = (a.asset_type || "").trim();
      ...
      <div className="w-64 shrink-0">
        <div className="panel p-3">
          ...
          <div className="space-y-1">
            {deviceGroups.map(([groupKey, groupAssets]) => {
              ...
              return (
                <div key={groupKey}>
                  ...
                  >
                    ...
                    <span className="text-xs text-ink-3">{groupAssets.length}개</span>
                  </button>
                  {open && (
