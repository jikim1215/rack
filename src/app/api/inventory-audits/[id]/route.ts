...
import type { InventoryAuditRow, CountRow } from "@/lib/db-types";

type Ctx = { params: Promise<{ id: string }> };

// 회차 마감(closed_at 기록) — 총괄(admin) 전용.
export const PUT = withApi(async (_req: NextRequest, { params }: Ctx) => {
  const actor = await getActor();
  assertAdmin(actor); // 조회보다 먼저 — 비관리자가 404/403 차이로 회차 id 를 열거하지 못하게 (비평 반영)
  ...
  const totalNow =
    (db.prepare("SELECT COUNT(*) c FROM assets").get() as CountRow).c +
    (db.prepare("SELECT COUNT(*) c FROM sub_assets WHERE status != 'disposed'").get() as CountRow).c;
  const checkedNow = (db.prepare("SELECT COUNT(*) c FROM inventory_audit_checks WHERE audit_id = ?").get(audit.id) as CountRow).c;
  // 스냅샷 확장 (외부 검토 R8-6 합의): 회차 단위 설명력 — 불일치(확인 외 판정) 수 + 장비/부속 확인 수
  const mismatchNow = (db.prepare("SELECT COUNT(*) c FROM inventory_audit_checks WHERE audit_id = ? AND result != 'confirmed'").get(audit.id) as CountRow).c;
  const equipNow = (db.prepare("SELECT COUNT(*) c FROM inventory_audit_checks WHERE audit_id = ? AND asset_id IS NOT NULL").get(audit.id) as CountRow).c;
  const subNow = (db.prepare("SELECT COUNT(*) c FROM inventory_audit_checks WHERE audit_id = ? AND sub_asset_id IS NOT NULL").get(audit.id) as CountRow).c;
  db.prepare(`
  ...
  const updated = db.prepare("SELECT * FROM inventory_audits WHERE id = ?").get(audit.id) as InventoryAuditRow;
