...
export const GET = withApi(async (req: NextRequest) => {
  ...
  if (["ip_format", "missing_id", "missing_os", "dup_suspect", "date_format"].includes(issueType)) { where.push("ii.issue_type = ?"); params.push(issueType); }
  if (batch) { where.push("ii.batch_id = ?"); params.push(batch); }
  const whereSql = where.length ? "WHERE " + where.join(" AND ") : "";

  const total = (db.prepare(`SELECT COUNT(*) AS c FROM import_issue ii ${whereSql}`).get(...params) as CountRow).c;
  const rows = db.prepare(`
    SELECT ii.*, a.asset_name
    FROM import_issue ii LEFT JOIN assets a ON ii.asset_id = a.id
    ${whereSql}
    ORDER BY ii.id DESC LIMIT ? OFFSET ?
  `).all(...params, limit, offset) as (ImportIssueRow & { asset_name: string | null })[];

  // 배치 요약 (필터바용): 배치별 건수·최초 시각·업로더
  interface BatchSummaryRow { batch_id: string; c: number; uploaded_at: string; uploaded_by: string }
...
export const PATCH = withApi(async (req: NextRequest) => {
  ...
  const id = int(b, "id", { required: true, min: 1, label: "id" });
  const status = oneOf(b, "status", ["open", "resolved", "ignored"] as const, { required: true, label: "status" });
  if (id == null) throw new ValidationError("id가 필요합니다.");
