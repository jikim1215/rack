...
import { VALID_STATUSES } from "@/lib/validation/asset-rules";
...
const STATUSES = new Set<string>(VALID_STATUSES);
const ZONE_MAXLEN = 30;

/** 요청 본문의 asset_ids 배열을 양의 정수 id 목록으로 검증한다. */
function parseAssetIds(body: Record<string, unknown>, label: string): number[] {
  if (!Array.isArray(body.asset_ids)) throw new ValidationError(`${label} 자산을 선택하세요.`);
  ...
  if (assetIds.length === 0) throw new ValidationError(`${label} 자산을 선택하세요.`);
  return assetIds;
}
...
export const PATCH = withApi(async (req: NextRequest) => {
  ...
  const assetIds = parseAssetIds(body, "수정할");
  ...
  if ("network_zone" in patch) {
    ...
    if (v.length > ZONE_MAXLEN) throw new ValidationError(`망구분은 최대 ${ZONE_MAXLEN}자입니다.`);
    setCols.push({ col: "network_zone", val: v });
  }
  if ("status" in patch) {
    const v = String(patch.status ?? "");
    if (!STATUSES.has(v)) throw new ValidationError("상태 값이 올바르지 않습니다.");
    setCols.push({ col: "status", val: v });
  }
  for (const col of ["admin_name", "user_name"] as const) {
    if (col in patch) {
      const v = String(patch[col] ?? "");
      if (v.length > 100) throw new ValidationError("값이 너무 깁니다(최대 100자).");
    ...
    if (col in patch) {
      ...
      if (raw === "" || raw == null) {
        ...
        if (!Number.isInteger(n) || n < 1 || n > 3) {
          throw new ValidationError("보안등급은 1~3 사이의 값이어야 합니다.");
        }
  ...
  if ("team_id" in patch) {
    ...
    if (raw === "" || raw == null) {
      ...
      const n = Number(raw);
      if (!Number.isInteger(n) || n <= 0) throw new ValidationError("관리부서 값이 올바르지 않습니다.");
      const exists = getDb().prepare("SELECT 1 FROM teams WHERE id = ?").get(n);
      if (!exists) throw new ValidationError("존재하지 않는 관리부서(팀)입니다.");
      setCols.push({ col: "team_id", val: n });
    }
  }
  if (setCols.length === 0) throw new ValidationError("변경할 항목을 선택하세요.");
  ...
  const targets = db
    ...
    .all(...assetIds, ...scope.params) as Array<Pick<AssetRow, "id" | "asset_name"> & Record<string, string | number | null>>;
...
export const DELETE = withApi(async (req: NextRequest) => {
  ...
  const body = asBody(await readJson(req));
  const assetIds = parseAssetIds(body, "삭제할");
  ...
  const targets = db
    ...
    .all(...assetIds, ...scope.params) as AssetRow[];
