...
export default function ContractsView({
  ...
  async function handleAddContract(e: React.FormEvent) {
    ...
    const res = await fetch("/api/contracts", {
      ...
      body: JSON.stringify({ ...cForm, vendor_id: cForm.vendor_id ? Number(cForm.vendor_id) : null, team_id: cForm.team_id === "" ? null : cForm.team_id }),
    });
    if (res.ok) {
  ...
  async function handleDeleteContract(id: number) {
    ...
    if (!confirm(`'${c?.contract_name || "이"}' 계약을 삭제하시겠습니까?\n\n자동 처리: 자산과의 계약 연결이 해제됩니다(자산 자체는 유지).\n삭제는 되돌릴 수 없습니다.`)) return;
  ...
  async function handleDeactivateVendor(id: number) {
    ...
    if (!confirm(`'${v?.vendor_name || "이"}' 업체를 비활성화하시겠습니까?\n\n자동 처리: 목록에서 숨겨지며 기존 계약의 업체 표기는 유지됩니다.`)) return;
  ...
  return (
    <div className="p-6">
      ...
      <UsageGuide
        ...
        items={[
          <>계약은 <strong className="text-ink-2">소유 팀별로 독립</strong> 관리됩니다. 팀 계정은 자기 팀 계약만, <strong className="text-ink-2">총괄</strong>은 전체를 봅니다. 계약 등록 시 팀 계정은 자동으로 자기 팀 소유가 됩니다</>,
          <>만료 <strong className="text-ink-2">60일 전부터 D-day 뱃지</strong>, 30일 전부터 행이 노랗게 표시됩니다. 목록은 만료 임박순 정렬입니다</>,
          <>업체 관리 탭에서 유지보수/공급 업체를 등록하고 계약과 연결합니다</>,
        ]}
      />
                    ...
                    return (
                      <tr key={c.id} className={`border-b border-line hover:bg-slate-100 ${contractRowClass(c)}`}>
                        <td className="px-4 py-2 font-medium">{c.contract_name}
                          {c.owner_team_name && <span className="ml-1 text-[10px] px-1 py-0.5 rounded bg-indigo-100 text-indigo-700 font-medium">{c.owner_team_name}</span>}
                        </td>
