...
insertUser.run("admin@example.go.kr", hashPw("admin123"), "시스템관리자", "admin");
insertUser.run("user@example.go.kr", hashPw("user123"), "일반사용자", "team");
insertUser.run("viewer@example.go.kr", hashPw("viewer123"), "열람자", "viewer");

// ── 실서버 최소 초기화: 스키마 + 계정 + 메뉴권한만 (데모 데이터 없음) ──
...
const sw3 = insertAsset.run("network", "액세스스위치-02", "Cisco", "Catalyst 2960X", "NET-2024-003", "10.10.0.3", "NW-003", "retired",
...
// 네트워크 그룹
const cf7 = insertField.run('additional_ips', '추가 IP', 'multi-text', '네트워크', '', 'server,network,security,telecom', 20, 0, 0, 1).lastInsertRowid;
