...
const insertAsset = db.prepare(`INSERT INTO assets 
  (asset_type, name, manufacturer, model, serial_number, ip_address, asset_tag, status, rack_id, rack_unit_start, rack_unit_size, description)
  VALUES (?,?,?,?,?,?,?,?,?,?,?,?)`);

// 서버
const srv1 = insertAsset.run("server", "웹서버-01", "Dell", "PowerEdge R740", "SRV-2024-001", "10.10.1.11", "SV-001", "active", rack1, 1, 2, "메인 웹서버").lastInsertRowid;
const srv2 = insertAsset.run("server", "웹서버-02", "Dell", "PowerEdge R740", "SRV-2024-002", "10.10.1.12", "SV-002", "active", rack1, 3, 2, "보조 웹서버").lastInsertRowid;
const srv3 = insertAsset.run("server", "DB서버-01", "HP", "ProLiant DL380 Gen10", "SRV-2024-003", "10.10.1.21", "SV-003", "active", rack1, 5, 2, "데이터베이스 서버").lastInsertRowid;
const srv4 = insertAsset.run("server", "백업서버", "Dell", "PowerEdge R640", "SRV-2024-004", "10.10.1.31", "SV-004", "active", rack1, 7, 1, "백업 서버").lastInsertRowid;
insertAsset.run("server", "개발서버", "HP", "ProLiant DL360 Gen10", "SRV-2024-005", "10.10.1.41", "SV-005", "active", rack4, 1, 2, "개발/테스트 서버");

// 네트워크
const sw1 = insertAsset.run("network", "코어스위치", "Cisco", "Catalyst 9300", "NET-2024-001", "10.10.0.1", "NW-001", "active", rack2, 1, 1, "L3 코어 스위치").lastInsertRowid;
const sw2 = insertAsset.run("network", "액세스스위치-01", "Cisco", "Catalyst 2960X", "NET-2024-002", "10.10.0.2", "NW-002", "active", rack2, 2, 1, "1층 액세스 스위치").lastInsertRowid;
const sw3 = insertAsset.run("network", "액세스스위치-02", "Cisco", "Catalyst 2960X", "NET-2024-003", "10.10.0.3", "NW-003", "active", rack2, 3, 1, "2층 액세스 스위치").lastInsertRowid;
insertAsset.run("network", "무선AP컨트롤러", "Aruba", "7010", "NET-2024-004", "10.10.0.10", "NW-004", "active", rack2, 4, 1, "무선 AP 컨트롤러");

// 보안
insertAsset.run("security", "방화벽", "Palo Alto", "PA-3260", "SEC-2024-001", "10.10.0.100", "SE-001", "active", rack3, 1, 2, "메인 방화벽");
insertAsset.run("security", "IPS", "AhnLab", "AIPS 4000", "SEC-2024-002", "10.10.0.101", "SE-002", "active", rack3, 3, 1, "침입방지시스템");
insertAsset.run("security", "웹방화벽", "Penta Security", "WAPPLES", "SEC-2024-003", "10.10.0.102", "SE-003", "active", rack3, 4, 1, "웹 애플리케이션 방화벽");
insertAsset.run("security", "NAC", "Genian", "NAC 5.0", "SEC-2024-004", "10.10.0.103", "SE-004", "active", rack3, 5, 1, "네트워크접근제어");

// 스토리지
insertAsset.run("storage", "SAN스토리지", "NetApp", "FAS2750", "STR-2024-001", "10.10.1.50", "ST-001", "active", rack1, 9, 4, "SAN 스토리지");
